...
public class ExcelUtil {
	...
	//操作到该excel的当前行数   保证线程安全使用ThreadLocal
	public static ThreadLocal<Integer> current_row = new ThreadLocal(){
		@Override
	...
	public static void createExcel(String year) {
		try {
			//每次请求时初始化为0
			current_row.set(0);
			String path = System.getProperty("user.dir")+File.separator+year+"年度获奖信息整理文档.xls";
			...
			//1.设置标题
			String title = year+"年大学生课外科技获奖统计";
			setTitle(sheet1,title);
			//设置第一行单元格高度
			 sheet1.setRowView(current_row.get(), title_height);
			...
			for (i = 0 ;i < types.size() ;i++) {
				...
				sheet1.setRowView(current_row.get(), small_title_height);
				current_row.set(current_row.get()+1);
				//3.设置列名   同样要current_row++
				setColumn(sheet1);
				sheet1.setRowView(current_row.get(), column_height);
				current_row.set(current_row.get()+1);
...
//			 title_wc.setBackground(jxl.format.Colour.RED); 
			...
			sheet.mergeCells(0,current_row.get(),columns_count-1, current_row.get());
			
			//新建单元格   四个参数分别代表单元格列数  单元格行数  单元格内容 单元格样式
			Label label = new Label(0,current_row.get(),title,title_wc);
	...
	public static void setSmallTitle(WritableSheet sheet,String title){
		try {
			...
			// title_wc.setBackground(jxl.format.Colour.RED); 
			sheet.mergeCells(0,current_row.get(),columns_count-1, current_row.get());
			Label label = new Label(0,current_row.get(),title,small_title_wc);
			sheet.addCell(label);	
		} catch (Exception e) {
			e.printStackTrace();
		} 
		...
		try {
	       ...
	       // jxl.write.WritableFont wfont = new jxl.write.WritableFont(WritableFont.createFont("宋体"), 12,WritableFont.BOLD); 
			...
			for (int i = 0; i < columns.length; i++) {
				Label label = new Label(i,current_row.get(),columns[i],column_wc);
				sheet.addCell(label);
			}
		} catch (Exception e) {
		}
	...
	public static void setValue(WritableSheet sheet,List<PrizeDetail> prizeDetails){
		try {
			for (int i = 0;i < prizeDetails.size();i++) {
				Label label = new Label(0,current_row.get(),(i+1)+"",content_wc);
				sheet.addCell(label);
				label = new Label(1,current_row.get(),prizeDetails.get(i).getAcademy(),content_wc);
				sheet.addCell(label);
				label = new Label(2,current_row.get(),prizeDetails.get(i).getClazz(),content_wc);
				sheet.addCell(label);
				label = new Label(3,current_row.get(),prizeDetails.get(i).getWorks_name(),content_wc);
				sheet.addCell(label);
				label = new Label(4,current_row.get(),prizeDetails.get(i).getPrize_name(),content_wc);
				sheet.addCell(label);
				label = new Label(5,current_row.get(),prizeDetails.get(i).getHost_name(),content_wc);
				sheet.addCell(label);
				label = new Label(6,current_row.get(),prizeDetails.get(i).getStudent_name(),content_wc);
				sheet.addCell(label);
				label = new Label(7,current_row.get(),prizeDetails.get(i).getPrize_price(),content_wc);
				sheet.addCell(label);
				current_row.set(current_row.get()+1);
			}
		} catch (Exception e) {
